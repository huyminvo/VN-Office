...
def _run_npm(directory):
  old_node_env = base.get_env("NODE_ENV")
  if ("production" == old_node_env):
    base.set_env("NODE_ENV", "development")
  retValue = base.cmd_in_dir(directory, "npm", ["install"], True)
  if (0 != retValue):
    retValue = base.cmd_in_dir(directory, "npm", ["install", "--verbose"])
  base.set_env("NODE_ENV", old_node_env)
  return retValue
...
def _run_npm_ci(directory):
  ...
  if ("production" == old_node_env):
    base.set_env("NODE_ENV", "development")
  retValue = base.cmd_in_dir(directory, "npm", ["ci"])
  base.set_env("NODE_ENV", old_node_env)
  return retValue
...
def _run_grunt(directory, params=[]):
  ...
  if ("windows" == base.host_platform()):
    local_grunt = directory + "/node_modules/.bin/grunt.cmd"
    if base.is_file(local_grunt):
      grunt = local_grunt
...
def _run_build_py(directory, params=[]):
  ...
  if (base.is_file(directory_result + "/Gruntfile.js")):
    grunt_params = []
    grunt_task = ""
    for param in params:
      if ("--minimize" == param):
      ...
      if ("--develop" == param):
        grunt_task = "develop"
        continue
      grunt_params.append(param)
    if ("" != grunt_task):
      grunt_params.append(grunt_task)
